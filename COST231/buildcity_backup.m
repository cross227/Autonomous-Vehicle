...
StreetsX=[];
for i=1:NoStreetsX,
    StreetsX=[StreetsX StreetsS*hroofs(ceil(rand*4))];
end
StreetsX=[StreetsX zeros(1,MapX-length(StreetsX))];

StreetsY=[];
for i=1:NoStreetsX,
    StreetsY=[StreetsY StreetsS*hroofs(ceil(rand*4))];
end
StreetsY=[StreetsY zeros(1,MapX-length(StreetsY))];
...
CoverageMap1 = CoverageMap;

%end for first antenna coverage;

%start of second antenna coverage calculation
posX = 0.9;
posY = 0.9;
...
%plotting graphs
figure(1)
imagesc(CoverageMap1) 
colorbar

figure(2)
imagesc(CoverageMap2) 
colorbar

CoverageMap = CoverageMap1 + CoverageMap2;
