function Coverage_matrix  = CoverageFcn(pos_antennas,CityMap,width,buildings)
%#codegen
...
[NoAntennas, ~] = size(pos_antennas);

CoverageMap_final = zeros(MapX,MapY);
...
for n=1:1:NoAntennas,
    ...
    if n==1
        CoverageMap_final = CoverageMap;    %updates final map
    else
        CoverageMap_final = max(CoverageMap_final,  CoverageMap);    %updates final map
    end
    
    disp(strcat('End of calculation for antenna #',int2str(n), '/',int2str(NoAntennas)))
end

CoverageMap_final = CoverageMap_final/NoAntennas;

Coverage_matrix = CoverageMap_final;
